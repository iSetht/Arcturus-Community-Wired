...
public class WiredEffectGiveCurrencyFromChest extends InteractionWiredEffect {
    ...
    public void execute(WiredContext ctx) {
        ...
        List<HabboItem> resolvedChests = ChestWiredUtil.onlyChests(chestManager, ChestWiredUtil.resolveItems(this, ctx, this.getFurniSource(), this.selectedItems, this.secondarySelectedItems), false, true);
        ...
        int requestedAmount = this.resolveAmount(ctx, resolvedChests);
        ...
        for (Habbo receiver : receivers) {
            ...
            int withdrawnForUser = 0;

            for (HabboItem chest : resolvedChests) {
                int amountFromChest = this.rewardingMode == MODE_ALL ? chestManager.getChestCoinBalance(chest) : remainingForUser;
                int given = chestManager.giveCoinsFromChest(ctx.room(), chest, receiver, amountFromChest);
                withdrawnCoins += given;
                withdrawnForUser += given;

                if (this.rewardingMode != MODE_ALL) {
                    remainingForUser -= given;
            ...
            if (withdrawnForUser > 0 && this.showRewardPopupByDefault && receiver.getClient() != null) {
                receiver.getClient().sendResponse(new ChestRewardPopupComposer(this.rewardText, withdrawnForUser, new ArrayList<>()));
            }
        }

        if (withdrawnCoins > 0) {
            ctx.state().setContextValue("@event.transaction_complete.withdrawal.coins_count", withdrawnCoins);
        }
    }
    ...
    public boolean saveData(WiredSettings settings, GameClient gameClient) throws WiredSaveException {
        ...
        if (params.length < 2) {
            throw new WiredSaveException("Invalid chest currency effect data");
        }
        ...
        this.saveFurniSource(settings, 6, WiredSources.SOURCE_TRIGGER);
        this.saveUserSource(settings, 7);
    ...
    private int resolveAmount(WiredContext ctx, List<HabboItem> resolvedChests) {
        if (this.rewardingMode == MODE_ALL) {
            int total = 0;
            ChestManager chestManager = Emulator.getGameEnvironment().getChestManager();
            for (HabboItem chest : resolvedChests) {
                total += chestManager.getChestCoinBalance(chest);
            }
            return total;
        }
