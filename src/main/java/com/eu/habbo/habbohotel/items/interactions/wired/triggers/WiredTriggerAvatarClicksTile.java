...
public class WiredTriggerAvatarClicksTile extends InteractionWiredTrigger {
    public static final WiredTriggerType type = WiredTriggerType.CLICK_TILE;
    ...
    public WiredTriggerAvatarClicksTile(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public boolean matches(HabboItem triggerItem, WiredEvent event) {
        ...
        // SOURCE_TILE_SELECTOR: check the clicked tile against all TilePicks selectors on this stack
        if (this.furniSource == WiredSources.SOURCE_TILE_SELECTOR) {
            RoomTile eventTile = event.getTile().orElse(null);
            if (eventTile == null) return false;
            for (InteractionWiredSelector selector : room.getRoomSpecialTypes().getSelectors(this.getX(), this.getY())) {
                if (selector instanceof WiredSelectorTilePicks) {
                    if (((WiredSelectorTilePicks) selector).containsTile(eventTile.x, eventTile.y)) {
                        return true;
                    }
                }
            }
    ...
    public void serializeWiredData(ServerMessage message, Room room) {
        ...
        // Detect if a TilePicks selector is on the same stack as this trigger
        boolean hasTilePicks = false;
        if (room != null) {
            for (InteractionWiredSelector selector : room.getRoomSpecialTypes().getSelectors(this.getX(), this.getY())) {
                if (selector instanceof WiredSelectorTilePicks) {
                    hasTilePicks = true;
        ...
        message.appendInt(hasTilePicks ? 1 : 0);
        message.appendInt(0);
    ...
    public boolean saveData(WiredSettings settings) {
        ...
        if (settings.getIntParams().length > 0) {
            this.furniSource = WiredSources.normalizeSource(settings.getIntParams()[0],
                    WiredSources.SOURCE_SELECTED, WiredSources.SOURCE_SELECTOR, WiredSources.SOURCE_TILE_SELECTOR);
        } else {
