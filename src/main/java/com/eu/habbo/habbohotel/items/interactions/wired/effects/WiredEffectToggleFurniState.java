...
public class WiredEffectToggleFurniState extends InteractionWiredEffect {
    private static final Logger LOGGER = LoggerFactory.getLogger(WiredEffectToggleFurniState.class);

    public static final WiredEffectType type = WiredEffectType.TOGGLE_STATE;
    private static final int TOGGLE_NEXT = 0;
    private static final int TOGGLE_PREVIOUS = 1;

    private final THashSet<HabboItem> items;
    private int toggleType = TOGGLE_NEXT;
    ...
    public WiredEffectToggleFurniState(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
        this.items = new THashSet<>();
    }

    public WiredEffectToggleFurniState(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public boolean saveData(WiredSettings settings, GameClient gameClient) throws WiredSaveException {
        ...
        int toggleType = settings.getIntParams().length > 1 ? settings.getIntParams()[1] : TOGGLE_NEXT;
        if (toggleType != TOGGLE_NEXT && toggleType != TOGGLE_PREVIOUS) {
            throw new WiredSaveException("Toggle type is invalid");
        }
    ...
    public void execute(WiredContext ctx) {
        ...
        boolean charged = ctx.event().getType() == WiredEvent.Type.RECEIVE_SIGNAL && this.getFurniSource() == WiredSources.SOURCE_SIGNAL
                ? WiredManager.getUsageTracker().tryConsumeCheapRuntimeItems(room, sourceItems.size())
        ...
        for (HabboItem item : sourceItems) {
            ...
            try {
                if (item.getBaseItem().getStateCount() > 1 || item instanceof InteractionGameTimer) {
                    ...
                    if (this.toggleType == TOGGLE_PREVIOUS && item.getBaseItem().getStateCount() > 1) {
                        int previousState = (state - 1 + item.getBaseItem().getStateCount()) % item.getBaseItem().getStateCount();
                        item.setExtradata(String.valueOf(previousState));
                        item.needsUpdate(true);
                        WiredManager.triggerNewFurniStateChange(room, item);
    ...
    private static int normalizeToggleType(Integer toggleType) {
        if (toggleType == null) {
            return TOGGLE_NEXT;
        }

        return (toggleType == TOGGLE_PREVIOUS) ? TOGGLE_PREVIOUS : TOGGLE_NEXT;
    }
    ...
    static class JsonData {
        ...
        public JsonData(int delay, List<Integer> itemIds, Integer toggleType) {
            this.delay = delay;
