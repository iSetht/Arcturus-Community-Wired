...
public class WiredConditionNotFurniHasFurni extends InteractionWiredCondition {
    public static final WiredConditionType type = WiredConditionType.NOT_HAS_FURNI_ON;

    private boolean all;
    private int furniSource = WiredSources.SOURCE_SELECTED;
    private THashSet<HabboItem> items;

    public WiredConditionNotFurniHasFurni(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
        this.items = new THashSet<>();
    }

    public WiredConditionNotFurniHasFurni(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public boolean evaluate(WiredContext ctx) {
        ...
        if (sourceItems.isEmpty() && this.furniSource == WiredSources.SOURCE_SELECTED)
            return true;
        ...
        if (this.all) {
            return sourceItems.stream().allMatch(item -> item != null && !hasFurniOnTop(item, room));
        } else {
            return sourceItems.stream().anyMatch(item -> item != null && !hasFurniOnTop(item, room));
        }
    }

    private boolean hasFurniOnTop(HabboItem item, Room room) {
        ...
        return occupiedTiles.stream().anyMatch(tile -> tile != null && room.getItemsAt(tile).stream().anyMatch(other -> other != item && other.getZ() >= minZ));
    }
