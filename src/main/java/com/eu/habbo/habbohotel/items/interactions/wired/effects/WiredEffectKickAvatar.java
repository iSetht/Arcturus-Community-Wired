...
public class WiredEffectKickAvatar extends InteractionWiredEffect {
    public static final WiredEffectType type = WiredEffectType.KICK_AVATAR;

    private String message = "";

    public WiredEffectKickAvatar(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredEffectKickAvatar(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
        ...
        for (RoomUnit roomUnit : this.resolveSourceUsers(ctx)) {
            ...
            if (habbo != null) {
                ...
                if (!this.message.isEmpty())
                    habbo.getClient().sendResponse(new RoomUserWhisperComposer(new RoomChatMessage(WiredTextPlaceholders.resolve(ctx, this.message), habbo, habbo, RoomChatMessageBubbles.ALERT)));
    ...
    public String getWiredData() {
        return this.withSourceData(WiredManager.getGson().toJson(new JsonData(this.message, this.getDelay())));
    }
