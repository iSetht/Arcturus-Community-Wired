...
public class WiredSelectorUserInNeighborhood extends InteractionWiredSelector {

    public static final WiredSelectorType type = WiredSelectorType.USER_IN_NEIGHBORHOOD;
    ...
    public WiredSelectorUserInNeighborhood(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
        this.selectedItemIds = new ArrayList<>();
        this.resetNeighborhood();
    }

    public WiredSelectorUserInNeighborhood(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public List<RoomUnit> getSelectedUsers(WiredEvent event) {
        ...
        for (RoomUnit roomUnit : room.getRoomUnits()) {
            if (roomUnit == null || !roomUnit.isInRoom()) {
                continue;
            }

            if (targetTiles.contains(new Point(roomUnit.getX(), roomUnit.getY()))) {
    ...
    private void resetNeighborhood() {
        ...
        this.originX = 0;
        this.originY = 0;
        this.sourceKind = SOURCE_KIND_USER;
